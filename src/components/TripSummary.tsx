...
import { Anchor, Calendar, MapPin, Sparkles, Ticket, Users } from 'lucide-react';
...
export function TripSummary() {
  ...
  const { destination, startDate, endDate, travelers, selectedActivities, portSchedule, getTotalCost } = useTripStore();
  ...
  return (
    <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="space-y-4">
        ...
        {portSchedule.length > 0 && <div className="space-y-2 rounded-3xl border border-slate-200 p-4"><div className="mb-2 flex items-center gap-2 text-sm font-medium text-slate-500"><Anchor size={16} /> {t('preferences.portItinerary')}</div>{portSchedule.map((port) => <div key={port.portIndex} className="flex items-center gap-2 rounded-2xl bg-slate-50 px-3 py-2"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-slate-200 text-[10px] font-bold text-slate-600">{port.portIndex}</span><div className="min-w-0"><p className="text-sm font-medium text-slate-800 truncate">{port.portName}</p><p className="text-xs text-slate-500">{port.date} • {port.dockArrival}–{port.dockDeparture}</p></div></div>)}</div>}
